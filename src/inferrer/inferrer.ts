...
function inferColorRules(tokens: Array<{ name: string; value: string; category: string }>): InferredRule[] {
  ...
  const semanticCount = colorTokens.filter((t) => semanticNames.some((s) => t.name.toLowerCase().includes(s))).length
  ...
  const baseColors = colorTokens.filter(
    (t) => !t.name.includes("hover") && !t.name.includes("active") && !t.name.includes("focus")
  )
...
function inferTypographyRules(tokens: Array<{ name: string; value: string; category: string }>): InferredRule[] {
  ...
  const fontFamilyTokens = fontTokens.filter(
    (t) => t.name.includes("font") && !t.name.includes("size") && !t.name.includes("weight")
  )

  if (fontFamilyTokens.length > 0) {
    const families = fontFamilyTokens.map((t) => t.value)
    const monoFonts = fontFamilyTokens.filter((t) => t.name.includes("mono") || t.value.toLowerCase().includes("mono"))
