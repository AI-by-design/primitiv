...
- \`get_component { name: "...", context: "<your working file or dir>" }\` — look up a specific component; pass your working path so same-name components resolve by scope. Add \`detail: "usage" | "relationships" | "all"\` only when static JSX-site evidence is needed. An \`ambiguous\` response carries an \`instruction\` — follow it (scope → \`rationale.when\` vs the user's intent → ask the user); never pick one arbitrarily
